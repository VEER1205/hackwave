...
const load = async (user) => {
  try {
    const balanceData = await API.get('/api/user/balance');
    STATE.balance = balanceData.balance || CONFIG.initialBalance;

    const portfolioData = await API.get('/api/portfolio');
    STATE.holdings = (portfolioData.holdings || []).map(h => ({
      type: h.type,
      symbol: h.symbol,
      id: h.symbol,
      name: h.name,
      qty: parseFloat(h.quantity),
      avg: parseFloat(h.avg_price),
      last: parseFloat(h.last_price || h.avg_price)
    }));

    const tradesData = await API.get('/api/trades?limit=100');
    STATE.trades = (tradesData.trades || []).map(t => ({
      ts: t.timestamp,
      type: t.type,
      side: t.side,
      symbol: t.symbol,
    ...
    STATE.watchlist = JSON.parse(localStorage.getItem(`tx:watch`) || JSON.stringify(['RELIANCE','TCS','INFY']));
    STATE.cryptoWatch = JSON.parse(localStorage.getItem(`tx:watchc`) || JSON.stringify(['bitcoin','ethereum','polygon-pos']));
    STATE.mfWatchlist = JSON.parse(localStorage.getItem(`tx:mfwatch`) || JSON.stringify(['HDFCBAL','PARAGFLEXI']));
...
function switchTab(tab) {
  ...
  if (tab === 'portfolio') computeAndRenderPerformance();
}
...
async function getSeriesCrypto(id, days=730) {
  ...
  if (STATE.seriesCache[key]) return STATE.seriesCache[key];
  const points = await fetchCryptoPriceSeries(id, days);
...
function calcCryptoChanges() {
  ...
  if (!d.length) {
    qs('#c24').textContent = qs('#c7d').textContent = qs('#c30d').textContent = qs('#c1y').textContent = '—';
    return;
  }
  const back = (days) => {
...
async function placeOrder(side, type) {
  ...
  const symbol = (type==='crypto') ? STATE.selectedCrypto.id
               : (type==='mf') ? STATE.selectedMF.symbol
               : STATE.selected.symbol;
               
  const name = (type==='crypto')
    ? (CRYPTO.find(c=>c.id===symbol)?.name || symbol)
    : (type==='mf')
      ? (DEMO_MF.find(m=>m.symbol===symbol)?.name || symbol)
      : (DEMO_STOCKS.find(s=>s.symbol===symbol)?.name || symbol);

  try {
    const result = await API.post('/api/trade', {
      type,
      side,
      symbol,
      name,
    ...
    alert(`${side} executed: ${symbol} x ${qty} @ ${currency(price)}`);
...
window.applyIPO = async function(code) {
  if (!STATE.user) {
  ...
  try {
    ...
    console.error('IPO application failed:', err);
    alert(err.message || 'IPO application failed');
  }
};
...
window.focusFromHistory = async function(type, symbol) {
  if (type==='stock') {
    ...
    switchTab('crypto');
    const id = (CRYPTO.find(c=>c.symbol===symbol)||{}).id || symbol.toLowerCase();
...
async function refreshPredictions() {
  ...
  const scored = [];
  for (const sym of candidates) {
    const ser = await getSeriesStock(sym, 260);
...
async function computeEquitySeries(startMs, endMs) {
  ...
  for (const sym of stockSyms) {
    ...
    const trades = STATE.trades
      .filter(t => t.type==='stock' && t.symbol===sym)
      .map(t => ({ ts: new Date(t.ts).getTime(), qty: (t.side==='BUY'?+1:-1)*t.qty, price: t.price }))
      .sort((a,b)=>a.ts-b.ts);
    assets.push({ key: sym, type:'stock', series, trades });
  }
  ...
  for (const id of cryptoIds) {
    ...
    const trades = STATE.trades
      .filter(t => t.type==='crypto' && t.symbol.toLowerCase()===id)
      .map(t => ({ ts: new Date(t.ts).getTime(), qty: (t.side==='BUY'?+1:-1)*t.qty, price: t.price }))
      .sort((a,b)=>a.ts-b.ts);
    assets.push({ key: id, type:'crypto', series, trades });
  }
  ...
  for (const sym of ipoSyms) {
    ...
    const trades = STATE.trades
      .filter(t => t.type==='ipo' && t.symbol===sym)
      .map(t => ({ ts: new Date(t.ts).getTime(), qty: (t.side==='BUY'?+1:-1)*t.qty, price: t.price }))
      .sort((a,b)=>a.ts-b.ts);
    assets.push({ key: sym, type:'ipo', series, trades });
  }
  ...
  for (const day of days) {
    while (tradeIdx < tradesAll.length && dayStart(tradesAll[tradeIdx].ts) <= day) {
      ...
      const key = (tr.type==='crypto') ? tr.sym.toLowerCase() : tr.sym;
...
on(qs('#addWatchBtn'), 'click', async () => {
  const s = (qs('#stockSearch').value || '').trim().toUpperCase();
  if (s && !STATE.watchlist.includes(s)) {
    STATE.watchlist.push(s);
    save();
    renderWatchlist();
  }
...
on(qs('#addWishBtn'), 'click', () => {
  const s = (qs('#stockSearch').value || '').trim().toUpperCase();
  if (s) addToWishlist(s);
});

on(qs('#wishFromHeader'), 'click', () => {
  const sym = STATE.selected?.symbol;
  if (sym) addToWishlist(sym);
});

on(qs('#clearWatch'), 'click', () => {
  STATE.watchlist = [];
  save();
  renderWatchlist();
});

on(qs('#clearWish'), 'click', () => {
  STATE.wishlist = [];
  save();
  renderWishlist();
});

on(qs('#clearMFWatch'), 'click', () => {
  STATE.mfWatchlist = [];
  save();
  renderMFWatchlist();
});

on(qs('#clearMFWish'), 'click', () => {
  STATE.mfWishlist = [];
  save();
  renderMFWishlist();
});

on(qs('#clearAllLists'), 'click', () => {
  if (confirm('Clear all watchlists and wishlists?')) {
    STATE.watchlist = [];
    STATE.wishlist = [];
    STATE.mfWatchlist = [];
    STATE.mfWishlist = [];
